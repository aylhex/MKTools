...
export const AppDecrypt: React.FC<AppDecryptProps> = ({ theme, platform: initialPlatform = 'android', deviceId, onError }) => {
  ...
  return (
    <div className={`flex h-full overflow-hidden ${isDark ? 'bg-[#1e1e20]' : 'bg-zinc-50/50'}`}>
      ...
      {contextMenu && (
        <div
          className={`fixed z-50 rounded-lg border shadow-xl py-1 min-w-[160px] ${isDark ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-slate-200'}`}
          style={{ left: contextMenu.x, top: contextMenu.y }}
          onClick={e => e.stopPropagation()}
        >
          <button
            className={`w-full px-4 py-2 text-left text-xs flex items-center gap-2 ${isDark ? 'hover:bg-zinc-700 text-zinc-200' : 'hover:bg-slate-100 text-slate-900'}`}
