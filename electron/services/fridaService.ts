...
// 兼容不同 su 实现，依次尝试多种语法
async function runAsRoot(adb: string, deviceId: string, cmd: string): Promise<string> {
  const suVariants = [
    `su -c '${cmd}'`,
    `su 0 -c '${cmd}'`,
    `su root -c '${cmd}'`,
  ...
  let lastError: any;
  for (const suCmd of suVariants) {
    try {
      const { stdout } = await execPromise(`"${adb}" -s "${deviceId}" shell "${suCmd}"`);
      return stdout;
    } catch (e: any) {
      // su: invalid uid/gid 或 Permission denied 时继续尝试下一种
      // su 本身报错（invalid uid/gid、failed to exec、Permission denied 等）时继续尝试下一种
      if (e.message && e.message.includes('su:')) {
        lastError = e;
        continue;
      }
      // 其他错误（如 pkill 找不到进程返回非0）直接抛出，让调用方处理
      throw e;
    }
  }
  throw lastError;
...
export async function checkAndDeployFridaServer(deviceId: string, platform: 'android' | 'ios', onLog: (msg: string) => void): Promise<boolean> {
  ...
  try {
    ...
    try {
      ...
      await runAsRoot(adb, deviceId, 'pkill -9 frida-server');
    ...
    await runAsRoot(adb, deviceId, `nohup ${remotePath} -D > /dev/null 2>&1 &`);
...
async function decryptAndroidApp(
  ...
  } catch (e) { /* grep 无结果时返回非0，正常 */ }

  onLog(`[Android] 准备启动应用进行脱壳`);

  // 2. 确定 Python 命令
  let pythonCmd: string;
  try {
    pythonCmd = await ensureFridaPythonModule(onLog);
  } catch (e: any) {
    onLog(`[Error] Python 环境异常: ${e.message}`);
    throw e;
  }

  // 3. 尝试 frida-dexdump 命令行工具（如果已 pip install）
  const tryCommandLineDexdump = async (): Promise<string> => {
    onLog('[Android] 尝试调用 frida-dexdump 命令...');
    const appOutputDir = path.join(outputDir, `${bundleId}_dex_${Date.now()}`);
    fs.mkdirSync(appOutputDir, { recursive: true });

    const args = ['-D', deviceId, '-f', bundleId, '--dump-all', '-o', appOutputDir];

    onLog(`[Android] 执行: frida-dexdump ${args.join(' ')}`);
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      child.on('error', (err: any) => {
        rejected = true;
        reject(new Error(`frida-dexdump command not found: ${err.message}`));
      });

      child.on('close', (code: any) => {
        if (rejected) return; // error 事件已处理
        try {
          const dexFiles = fs.readdirSync(appOutputDir).filter(f => f.endsWith('.dex'));
          outputCount = dexFiles.length;
          onLog(`[Android] 找到 ${dexFiles.length} 个 DEX 文件`);
        } catch (e) {}

        if (outputCount > 0) {
          resolve(appOutputDir);
        } else {
          reject(new Error(`frida-dexdump exited with code ${code}, no DEX files found`));
        }
      });
      ...
      setTimeout(() => {
        child.kill();
        reject(new Error('frida-dexdump timeout (5 min)'));
      }, 5 * 60 * 1000);
    });
  };

  // 4. 自定义内联 Python 脚本（仅依赖 frida 模块，无其他依赖）
  const tryCustomDexdump = async (): Promise<string> => {
    const appOutputDir = path.join(outputDir, `${bundleId}_dex_${Date.now()}`);
    fs.mkdirSync(appOutputDir, { recursive: true });

    onLog('[Android] 使用内置 DEX 内存扫描方案...');
...
def get_main_pid(timeout=30):
    """Poll adb shell pidof to find the main process PID (exact package name match)."""
    for attempt in range(timeout):
        try:
            result = subprocess.run(
                [ADB, "-s", DEVICE_ID, "shell", "pidof", BUNDLE_ID],
                capture_output=True, text=True, timeout=5
            )
            parts = result.stdout.strip().split()
            if parts and parts[0].isdigit():
                return int(parts[0]), attempt + 1
        except Exception:
            pass
        time.sleep(1)
    return None, timeout
...
# ── Strategy 1: Frida spawn mode ────────────────────────────────────────────
try:
    print(f"[dexdump] Strategy 1: Spawning {BUNDLE_ID} via Frida...", file=sys.stderr)
    spawned_pid = device.spawn(BUNDLE_ID)
    session = device.attach(spawned_pid)
    device.resume(spawned_pid)
    print(f"[dexdump] Spawned PID {spawned_pid}, waiting 10s for unpack...", file=sys.stderr)
    time.sleep(10)
    print(f"[dexdump] Starting DEX scan (spawn mode)...", file=sys.stderr)
except Exception as e:
    print(f"[dexdump Warn] Spawn failed: {e}", file=sys.stderr)
    print(f"[dexdump] Switching to adb-launch + attach mode...", file=sys.stderr)
    if session:
        try:
            session.detach()
        except Exception:
...
(function() {
    ...
    function tryDump(base, size) {
        try {
            ...
            if (fileSize < 0x70 || fileSize > size) return;
            var data = Memory.readByteArray(base, fileSize);
            if (data) {
                send({t: 'dex', addr: base.toString(), sz: fileSize}, data);
                saved.push(base.toString());
            }
        } catch(e) {}
    }
    ...
    Process.enumerateRanges('r--').forEach(function(r) {
        try {
            Memory.scanSync(r.base, r.size, '64 65 78 0a').forEach(function(m) {
                tryDump(m.address, r.size - m.address.sub(r.base).toInt32());
            });
        } catch(e) {}
    });

    // Also try r-x ranges (some loaders mark DEX as executable)
    ...
    send({t: 'done', count: saved.length});
})();
"""

def run_scan(sess):
    """Inject the DEX scanner into sess and wait for results. Returns True on success."""
    dex_index = [0]
    done_event = [False]
    ...
    def on_message(message, data):
        if message.get('type') == 'send':
            p = message['payload']
            if p.get('t') == 'dex' and data:
                idx = dex_index[0]
                fname = 'classes.dex' if idx == 0 else f'classes{idx}.dex'
                fpath = os.path.join(OUTPUT_DIR, fname)
                with open(fpath, 'wb') as f:
                    f.write(data)
                print(f"[dexdump] Saved {fname} ({len(data)} bytes, addr={p['addr']})", file=sys.stderr)
                dex_index[0] += 1
            elif p.get('t') == 'done':
                print(f"[dexdump] Scan complete, {p['count']} DEX regions found", file=sys.stderr)
                done_event[0] = True
                print('DEXDUMP_COMPLETE', flush=True)
        elif message.get('type') == 'error':
            desc = message.get('description', '')
            if 'access violation' not in desc.lower():
                print(f"[dexdump Warn] {desc}", file=sys.stderr)
...
# ── Strategy 1: try scan with spawn session ──────────────────────────────────
if session is not None:
    try:
        run_scan(session)
        sys.exit(0)
    except Exception as e:
        print(f"[dexdump Warn] Spawn session lost: {e}", file=sys.stderr)
        print(f"[dexdump] Falling back to adb-launch + attach mode...", file=sys.stderr)
        try:
            session.detach()
        except Exception:
            pass
...
# ── Strategy 2 (fallback): kill + adb launch + attach ───────────────────────
if session is None:
    try:
        print(f"[dexdump] Force-stopping {BUNDLE_ID}...", file=sys.stderr)
        subprocess.run(
        ...
        print(f"[dexdump] Launching {BUNDLE_ID} via adb monkey...", file=sys.stderr)
        subprocess.Popen(
        ...
        found_pid, elapsed = get_main_pid(timeout=30)
        if not found_pid:
            print(f"[dexdump Error] Process {BUNDLE_ID} not found after 30s", file=sys.stderr)
            sys.exit(1)

        print(f"[dexdump] Found PID {found_pid} after {elapsed}s, waiting 10s for unpack...", file=sys.stderr)
        time.sleep(10)

        print(f"[dexdump] Attaching to PID {found_pid}...", file=sys.stderr)
        session = device.attach(found_pid)
        print(f"[dexdump] Attached (adb mode). Starting DEX scan...", file=sys.stderr)
        run_scan(session)
    except Exception as e:
        print(f"[dexdump Error] Attach/scan failed: {e}", file=sys.stderr)
        sys.exit(1)
`;

    const tmpScript = path.join(os.tmpdir(), `dexdump_${Date.now()}.py`);
    await fs.promises.writeFile(tmpScript, pythonScript);

    return new Promise<string>((resolve, reject) => {
      const { spawn } = require('node:child_process');
      const child = spawn(pythonCmd, [tmpScript]);
      let outputCount = 0;

      child.stdout.on('data', (_data: any) => {
        // stdout 仅用于内部协议标记（DEXDUMP_COMPLETE），不输出到日志
      });
      ...
      child.on('close', (code: any) => {
        ...
        try {
          const dexFiles = fs.readdirSync(appOutputDir).filter(f => f.endsWith('.dex'));
          outputCount = dexFiles.length;
          onLog(`[Android] 共提取 ${dexFiles.length} 个 DEX 文件`);
        } catch (e) {}

        if (outputCount > 0) {
          resolve(appOutputDir);
        } else {
          reject(new Error(`Custom dexdump exited with code ${code}, no DEX files found`));
        }
      });
      ...
      setTimeout(() => {
        child.kill();
        fs.unlink(tmpScript, () => {});
        reject(new Error('Custom dexdump timeout (5 min)'));
      }, 5 * 60 * 1000);
    });
  };

  // 5. 依次尝试：命令行工具 → 内置内存扫描 → 基础 APK 提取
  try {
    const result = await tryCommandLineDexdump();
    onLog(`[✓] DEX 文件已保存到: ${result}`);
    return result;
  } catch (e1: any) {
    onLog(`[Android] frida-dexdump 命令不可用 (${e1.message})，切换到内置方案...`);
  }

  try {
    const result = await tryCustomDexdump();
    onLog(`[✓] DEX 文件已保存到: ${result}`);
    return result;
  } catch (e2: any) {
    onLog(`[Android] 内存扫描失败 (${e2.message})，回退到基础 APK 提取...`);
    onLog(`[Android Warn] 基础提取的 APK 可能仍有加密壳`);
  }
